...
1.  **Client Request**: A client sends a request to an OpenAI endpoint (e.g., `/v1/chat/completions`).
2.  **Authorization**: The gateway validates the `Authorization: Bearer <key>` header.
3.  **Routing Decision**:
    *   **Explicit Model**: If `model` is specified as a known model name, it bypasses routing.
    *   **Automatic Routing**: If `model: "auto"` or no model is provided, the **LLM Classifier** analyzes the conversation history and selects the best route (e.g., `coder`, `chat`).
4.  **Model Management**: The `ModelManager` checks if the target model's server is running. If not, it executes the configured `start_command`.
5.  **Execution & Proxying**: The request is proxied to the upstream server (local or remote).
6.  **Session Management**: If an `X-Session-ID` header is present, conversation history is persisted locally for future context.
7.  **Cleanup**: After a period of inactivity, the model is unloaded to free up VRAM.
...
When a request is sent to `/v1/images/generations`:
1.  **Refinement**: If enabled, the gateway uses your configured LLM to expand or improve user prompts before sending them to the image backend.
2.  **Execution**: The system invokes `sd-cli` with the refined prompt and configured weights (diffusion model, VAE, CLIP).
3.  **Output**: Results are returned as base64 encoded images in the standard OpenAI JSON format.
...
```text
router/
├── ai-gateway/           # Core Gateway Implementation
│   ├── api/              # FastAPI route handlers (OpenAI, Health, Images, Tools)
│   ├── llm/              # Intent classification and prompt refinement logic
│   ├── managers/         # Lifecycle management (Models, Sessions, Routing)
│   ├── models/           # Data structures and model registry
│   ├── prompts/          # System prompt templates
|   └── tools/            # Tool execution logic (Git, Docker, etc.)
├── .agents/              # Agentic workflow configurations
...
client = openai.OpenAI(
    ...
    api_key="your-secure-secret-here"
)

# Using 'auto' to let the gateway route between chat and coder models
...
curl http://localhost:8000/v1/images/generations \
  ...
  -d '{"prompt":"a futuristic cyberpunk city in the rain", "size":"1024x1024"}'
```
